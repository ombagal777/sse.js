...
var SSE = function (url, options) {
  ...
  /** @type {number} */
  this._baseReconnectDelay = this.reconnectDelay;

  /** @type {string} */
  this.reconnectDelayStrategy = options.reconnectDelayStrategy || "fixed";
  if (
    this.reconnectDelayStrategy !== "fixed" &&
    this.reconnectDelayStrategy !== "exponential" &&
    this.reconnectDelayStrategy !== "jitter" &&
    this.reconnectDelayStrategy !== "custom"
  ) {
    this.reconnectDelayStrategy = "fixed";
  }

  /** @type {Function|null} */
  this.customReconnectDelay = options.customReconnectDelay || null;

  /** @type {number|null} */
  this.maxReconnectDelay =
    options.maxReconnectDelay !== undefined ? options.maxReconnectDelay : null;

  /** @type {number|null} */
  this._serverRetryDelay = null;

  /** @type {number|null} */
  this._lastReconnectDelay = null;
  ...
  /** @private */
  this._getReconnectDelay = function () {
    let delay = this._baseReconnectDelay;

    if (this.reconnectDelayStrategy === "exponential") {
      delay = this._baseReconnectDelay * Math.pow(2, this.retryCount);
    } else if (this.reconnectDelayStrategy === "jitter") {
      delay =
        this._baseReconnectDelay +
        Math.round(this._baseReconnectDelay * Math.random());
    } else if (
      this.reconnectDelayStrategy === "custom" &&
      this.customReconnectDelay
    ) {
      delay = this.customReconnectDelay({
        retryCount: this.retryCount,
        baseDelay: this._baseReconnectDelay,
        lastDelay: this._lastReconnectDelay,
        serverRetryDelay: this._serverRetryDelay,
    ...
    if (this._serverRetryDelay !== null) {
      delay = Math.max(delay, this._serverRetryDelay);
    }
  ...
  this._markClosed = function () {
    ...
    if (this.autoReconnect) {
      ...
      if (this.debug) {
        console.debug(
          `SSE will attempt to reconnect in ${reconnectDelay}ms (attempt ${this.retryCount + 1
          }${this.maxRetries ? "/" + this.maxRetries : ""})`
        );
      }

      const attemptEvent = new CustomEvent("reconnect-attempt");
      attemptEvent.retryCount = this.retryCount;
      attemptEvent.delay = reconnectDelay;
      attemptEvent.strategy = this.reconnectDelayStrategy;
      attemptEvent.serverRetryDelay = this._serverRetryDelay;
      ...
      this._pendingTelemetry = {
        ...
        strategy: this.reconnectDelayStrategy,
        serverRetryDelay: this._serverRetryDelay,
      };
  ...
  this._onStreamProgress = function (e) {
    ...
    if (this._reconnectInProgress) {
      const t = this._pendingTelemetry;
      const succeededEvent = new CustomEvent("reconnect-succeeded");
      succeededEvent.retryCount = t.retryCount;
      succeededEvent.delay = t.delay;
      succeededEvent.strategy = t.strategy;
      succeededEvent.serverRetryDelay = t.serverRetryDelay;
      this.dispatchEvent(succeededEvent);
      this._reconnectInProgress = false;
      this._pendingTelemetry = null;
    }
    this.retryCount = 0;
    this._serverRetryDelay = null;
    this.reconnectDelay = this._baseReconnectDelay;
  ...
  this._parseEventChunk = function (chunk) {
    ...
    if (e.retry !== null && /^[0-9]+$/.test(e.retry)) {
      const retryDelay = parseInt(e.retry, 10);
      this.reconnectDelay = retryDelay;
      this._serverRetryDelay = retryDelay;
    }
 ...
 * @property {'fixed'|'exponential'|'jitter'|'custom'} [reconnectDelayStrategy] - strategy for calculating reconnect delays
 * @property {function(context: ReconnectDelayContext): number} [customReconnectDelay] - custom delay callback (strategy: "custom")
 * @property {number|null} [maxReconnectDelay] - maximum reconnect delay cap
 ...
 */
/**
 * @typedef {Object} ReconnectDelayContext
 * @property {number} retryCount - current retry attempt count
 * @property {number} baseDelay - base reconnect delay in ms
 * @property {number|null} lastDelay - delay used in the previous attempt
 * @property {number|null} serverRetryDelay - server-provided retry delay floor
 * @property {number|null} maxReconnectDelay - maximum reconnect delay cap
 */
/**
 ...
 * @property {number} reconnectDelay - delay in ms before reconnecting
 * @property {string} reconnectDelayStrategy - strategy for calculating reconnect delays
 * @property {number|null} maxReconnectDelay - maximum reconnect delay cap
 * @property {number|null} maxRetries - maximum number of reconnect attempts
 * @property {boolean} useLastEventId - flag, if Last-Event-ID header should be sent
